...
function PartnerRegister() {
    ...
    const { storePartnerToken } = useAuth()
    ...
    const sendOTP = async (e) => {
        ...
        try {
            setLoadingBtn(true)
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/partner/sendotp`, {
                method: "POST",
                headers: {
    ...
    const verifyOtp = async (e) => {
        ...
        try {
            ...
            const verifyResponse = await fetch(`${import.meta.env.VITE_API_URL}/api/partner/verifyotp`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
            ...
            if (verifyResponse.ok) {
                if (verifyResponseData.otp.otp === Number(otp)) {
                    const response = await fetch(`${import.meta.env.VITE_API_URL}/api/partner/register`, {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json"
                        },
                    ...
                    if (response.ok) {
                        storePartnerToken(responseData.token)
        ...
        <>
            <div className='flex flex-wrap justify-center items-center h-auto py-10'>
                <div className="w-96 mx-auto bg-white p-8 rounded-2xl shadow-none lg:shadow-md">
                    <h1 className="text-3xl text-black font-bold mb-6 flex flex-wrap justify-center text-center">Register as Delivery Partner</h1>
                    <h3 className="text-gray-700">Already registered? <Link className='font-bold text-[#FFB500]' to="/partner-login">Login</Link></h3>
                    <form>
