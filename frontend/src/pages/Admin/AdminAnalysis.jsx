...
const AdminAnalysis = () => {
    ...
    const getOrdersOfCurrentWeek = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/order/get-all-orders-current-week`, {
                method: "GET",
                headers: {
    ...
    const getOrdersLastSixMonthsByCompany = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/order/get-all-orders-of-months`, {
                method: "GET",
                headers: {
                    "Content-Type": "applicaation/json",
                    "Authorization": `Bearer ${adminToken}`
